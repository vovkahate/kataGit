class View {
  ...
  addRepoData(name, owner, stars) {
    ...
    this.cross.addEventListener("click", (e) => {
      if (confirm("Вы уверены?")) e.target.parentElement.remove();
    });
...
class Search {
  ...
  async searchUsers() {
    ...
    if (
      this.view.searchInput.value[this.view.searchInput.value.length - 1] ===
      " "
    ) {
      ...
      return await fetch(
        `https://api.github.com/search/repositories?q=${this.view.searchInput.value.trim()}&per_page=5`
      ).then((response) => {
        if (response.ok) {
  ...
  clearUsers() {
    ...
    this.view.searchCounter.textContent =
      "начните вводить название репозитория";
  }
